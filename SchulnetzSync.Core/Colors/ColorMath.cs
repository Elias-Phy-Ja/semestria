...
/// <summary>An sRGB colour with 8 bits per channel.</summary>
public readonly record struct RgbColor(byte R, byte G, byte B)
{
    ...
    /// <summary>
    /// Parses <c>#RRGGBB</c>, <c>RRGGBB</c>, <c>#RGB</c> or <c>RGB</c>.
    /// </summary>
    /// <returns>False for anything else, including alpha values.</returns>
    public static bool TryParseHex(string? text, out RgbColor color)
    {
        ...
        // Kurzform #RGB wie in CSS: jede Ziffer verdoppeln
        if (s.Length == 3)
...
///
/// Ein Farbmischer arbeitet in HSV statt RGB, weil man dort intuitiv mischt:
/// ein Regler für den Farbton, eine Fläche für «wie kräftig» und «wie hell».
/// </summary>
/// <param name="H">Hue in degrees, 0 up to (not including) 360.</param>
/// <param name="S">Saturation, 0 (grey) to 1 (full colour).</param>
/// <param name="V">Value, 0 (black) to 1 (full brightness).</param>
public readonly record struct HsvColor(double H, double S, double V)
{
    public RgbColor ToRgb()
    {
        double h = ((H % 360) + 360) % 360;   // auch negative oder 360 sauber einordnen
